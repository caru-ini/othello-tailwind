...
const Menu = ({ onReset, player, count }: menuProps) => {
    ...
    return (
        ...
        <div className="flex md:justify-between items-center gap-3">
            <div className={`aspect-square rounded-full drop-shadow-md w-12 ${playerColor[player]}`}>
            </div>
            {/* title */}
            <div className="flex flex-col">
            <h1 className="text-2xl font-bold"><span className="text-cyan-500">INIAD</span>.ts</h1>
            <h1 className="text-2xl font-bold">Reversi</h1>
            </div>
        </div>
        <div className="flex gap-5 items-center justify-evenly">
            <Button variant={"ghost"} className="px-2" onClick={onReset}><RotateCcw /></Button>
            {/* count */}
